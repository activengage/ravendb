...
namespace Raven35.Tests.Bundles.Authorization
{
    public class CanAskAuthQuestionsFromServer : AuthorizationTest
    {
        ...
        public void CanAskWhateverUserHavePermissions()
        {
            ...
            using (var s = store.OpenSession(DatabaseName))
            {
                var isOperationAllowedOnDocument = client::Raven35.Client.Authorization.AuthorizationClientExtensions.IsOperationAllowedOnDocument(s.Advanced, UserId, "Company/Bid", "companies/1");
